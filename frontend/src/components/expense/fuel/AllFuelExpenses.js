...
function AllFuelExpenses() {
  ...
  const generateFilteredExcelData = () => {
    const myPromise = getFilteredFuelExpenses(startDate, endDate, 0, 0);
  ...
  useEffect(() => {
    if (isFiltered) {
      getFilteredFuelExpenses(startDate, endDate, documentLimit, skipCount)
        .then((res) => {
          if (res.data.success) {
      ...
      getAllFuelExpenses(documentLimit, skipCount)
        .then((res) => {
          if (res.data.success) {
  ...
  return (
    <section className="p-5 md:p-10 bg-gray-100 min-h-screen">
      <section class="container px-4 mx-auto">
        <h2 className="text-xl mb-4 font-semibold">Fuel Expenses</h2>
        ...
        {excelFullData.length > 0 && (
          <CSVLink ref={csvRef} data={excelFullData} filename="Fuel Expenses" />
        )}
        {filteredExcelFullData.length > 0 && (
          <CSVLink
            ref={csvRef}
            data={filteredExcelFullData}
            filename={`${startDate} to ${endDate} Fuel Expenses`}
          />
        )}
